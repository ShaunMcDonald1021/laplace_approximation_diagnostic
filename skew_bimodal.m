...
f = @(x) normpdf(x(:,2)).*(0.7*sqrt(2)*exp(-x(:,1).^2) + 2*0.3*exp(-(x(:,1)-2).^2/2).*normcdf(6*(x(:,1)-2)))/sqrt(2*pi);
% [mode, ~, f_at_mode, hess_at_mode] = laplace_ingredients(f, d,
% false);
mode = [0 0];
hess_at_mode = diag([-1.9999999999999999999999999999992 -1]);
f_at_mode = f(mode);
...
T = diag([sqrt(0.5) 1]); % See Section 4.1 of manuscript
s = s_star*T + mode;
f_interrs = f(s);
interrs = (f_interrs - gauss_approx(s))./g(s);

% Set up 2D grid to plot functions
[xgrid_star, ygrid_star] = meshgrid(-4:0.05:7, -5.5:0.05:5.5);
...
post_mean = 0.7 + int_weightz'*pointz; % Posterior integral mean
...
legend([true_surf, gauss_surf], {'$f$', 'Gaussian approximation'},...
...
legend([post_surf, true_surf2, interr_points],...
    {'$m^x_1 \cdot g$', '$f$', 'Interrogation points'},...
    'Location', 'north', 'Interpreter', 'latex', 'FontSize', 20);
hold off
...
set(gca, 'FontSize', 20)
set(gca, 'TickLabelInterpreter', 'latex')
lap_app = xline(0.7, '--g','LineWidth', 1.25);
...
quant_line = xline(norminv(0.975, post_mean, sqrt(post_var)),...
    ':k', 'LineWidth', 1.5);
legend([lap_dist, quant_line, lap_app, true_line, post_line],...
    {'Integral posterior', '95\% interval', 'LA',...
    'True integral value', '$m_1$'},...
